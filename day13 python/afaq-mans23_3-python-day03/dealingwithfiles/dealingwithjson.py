...
def writeTojson():
    ...
    try:
        ...
        data_to_write = json.dumps(obj,indent='2')
...
def writeTojson():
    ...
    try:
        fileobj = open('users2.json', 'w')
    except Exception as e:
        print(e)
        return False
    else:
        # convert dict to string
        data_to_write = json.dumps(old_data,indent=2) ## indent 2 align data to suitable jason format
        print(data_to_write, type(data_to_write))
        fileobj.write(data_to_write)
        fileobj.close()
